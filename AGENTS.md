...
**Windows console encoding is not evidence**: PowerShell/stdout may display UTF-8 CJK/Japanese text as `??` or mojibake even when the JSON/image bytes are correct. Do not conclude text corruption from console display alone. Verify text artifacts by reading bytes as UTF-8 (`Path.read_bytes().decode("utf-8")`, `json.loads(...)`, or codepoint/`repr` inspection), and verify rendered text by opening/cropping the PNG or using a vision-capable model. Avoid piping non-ASCII test fixtures through PowerShell here-strings/stdin; use `\uXXXX` escapes in temporary Python verification scripts or a UTF-8 file read with an explicit encoding.
