"""Mimo provider for LLM-powered manga translation."""
...
DEFAULT_VISION_MODEL = "mimo-v2.5-pro"
...
def _content(response: Any) -> str:
    ...
    if c is None:
        raise ProviderResponseError("Mimo returned empty content")
    return c
...
class MimoProvider(LLMProvider):
    """Concrete LLM provider backed by the Mimo API.

    Mimo is a Chinese AI provider with vision capabilities for manga translation.
    ...
    def __init__(
        ...
        *,
        ...
        resolved_base_url = base_url or os.getenv(base_url_env or "") or DEFAULT_BASE_URL
    ...
    def cost_per_1k_tokens(self) -> Optional[float]:
        return None  # Mimo pricing not publicly available
    ...
    def vision_extract(self, page: Page, *, store: Optional[ArtifactStore] = None) -> tuple[Page, dict]:
        """Extract text bubbles from a manga page image via Mimo vision."""
    ...
    def translate(
        ...
        """Translate utterances for a page using Mimo."""
        payload = {
