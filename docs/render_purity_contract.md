...
4. **`I_n`-faithful render decisions (anti-hallucination, PRD §3.1 P1).** The OCR hallucination guard drops translations whose **OCR** region lands on a blank area of `I_n`. This is a *render decision* (what to render), not a geometry override — it does not alter the authoritative artifact. Vision-injected seats are intentionally excluded from the OCR blank-region guard because real contents-page text can sit on a white background; applying the OCR guard there deletes valid TOC translations. Vision injection is conservative, not fallback-only: `_inject_vision_regions` reuses already-tagged vision seats, skips vision bubbles whose source text already exists in OCR, skips vision bboxes that overlap OCR geometry, and only appends seats for uncovered vision-only bubbles. For contents pages, OCR geometry stays authoritative even when OCR only saw the chapter number; if the following vision entry supplies the missing title, render supplements the text in the same OCR seat instead of inventing a new bbox. This avoids the recon-fresh-v4 white-box/text-outside-bubbles regression while still rendering mixed OCR+vision pages where OCR missed a bubble.
...
4. **Conservative mixed vision injection, artifact/inpaint pruning, and RGB output verified** (2026-06-27) -> full 10-page e2e on `data/input/test-pdf-10pages` with visual inspection of the rendered PNGs. `page-007` no longer leaks text on the left/left-bottom bubbles because the runtime target polygon is kept bounded after font fitting. `page-009` no longer renders SFX/vision text over the character art because non-renderable vision box types are blocked from render seats. `page-010` no longer has mass empty speech bubbles. Pages suppressed to zero rendered translations also have their artifact text regions pruned and Pass 1 inpainted pixels restored, so cover/contents/chapter pages do not turn into white holes. The final PIL output path preserves RGB channel order, so color pages no longer render blue-tinted. Output: `data/output/e2e-overflow-fix-20260627-v5`.
5. **Contents title supplement and explicit footnotes verified** (2026-06-28) -> render-only proof on `data/output/e2e-render-current-20260628-footnotes-on-v2`. `page-004` renders `第34话 恸愧之鞭` in the OCR chapter-number seat by supplementing the OCR text from the following vision title while keeping OCR geometry authoritative. `page-005` and `page-008` both render translated text. Default render omits page footnotes; explicit `render_footnotes=True` renders readable CJK footers on pages 005, 009, and 010.
...
- On render, `resolve_pinned_artifact` restores the pinned artifact for `I_n` before the guard and subprocess run — so a fresh re-OCR that overwrites the artifact does **not** change the rendered output. A cached artifact with fewer valid text regions than the fresh artifact is not restored; the pin is upgraded instead. This prevents stale empty pins from erasing source text while leaving no render seats (the `page-010` empty-bubble regression). The render stage also replaces a stale wrong-page pin when the current artifact's source texts match the current page and the pin's texts do not.
...
| `test_empty_pin_does_not_override_current_non_empty_artifact` | A stale empty pin is upgraded to the fresh non-empty artifact instead of suppressing render seats |
| `test_same_source_vision_region_skipped_even_when_bbox_misses_ocr` | A duplicate vision bubble cannot render outside the speech bubble when OCR already has the same source text |
| `test_near_same_source_vision_region_skipped_for_punctuation_drift` | A page-007-style OCR/vision duplicate with punctuation drift cannot create an out-of-bubble vision seat |
| `test_contents_chapter_number_uses_following_vision_title` | A contents-page OCR region containing only a chapter number is rendered with the following vision title in the same OCR geometry |
| `test_empty_final_translations_clear_artifact_and_restore_inpainted_image` | Suppressing every translation also clears artifact seats and restores Pass 1 inpainted pixels from the input image |
| `test_artifact_cache.py::*` (10 tests) | `image_sha256` determinism, `ArtifactPin` get/put/persistence, `resolve_pinned_artifact` first-seen-wins + restore + empty-pin recovery |
